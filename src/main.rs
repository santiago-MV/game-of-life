...
async fn main() {
    ...
    loop {
        ...
        let neighbour_grid = get_neighbour_amount_grid(&grid);
            ...
            for col in row {
                if grid[row_index][col_index] == 1{
                    match *col {
                        ...
                        3..=8 => grid[row_index][col_index] = 0,
                        _ => continue,
                    }
                }
                else{
...
fn get_neighbour_amount_grid(grid:&Vec<Vec<i32>>) -> Vec<Vec<i32>>{
    ...
    for row in grid{
        ...
        for _ in row{
            ...
            match row_index {
                ...
                _ => neighbour_amount       += grid[row_index+1][column_index]  //ADD both
            ...
            match column_index {
                ...
                _ => neighbour_amount       += grid[row_index][column_index+1]  //ADD both
            ...
            match (row_index,column_index){
                ...
                (0,_) => neighbour_amount               += grid[row_index+1][column_index+1]    //ADD both DOWN RIGHT and LEFT
                                                        +  grid[row_index+1][column_index-1],
                (MAX_ROW,_) => neighbour_amount         += grid[row_index-1][column_index+1]    //ADD both UP RIGHT and LEFT
                                                        +  grid[row_index-1][column_index-1],
                (_,0) =>        neighbour_amount        += grid[row_index+1][column_index+1]    //ADD both RIGHT UP and DOWN
                                                        +  grid[row_index-1][column_index+1],
                (_,MAX_COL) => neighbour_amount         += grid[row_index+1][column_index-1]    //ADD both LEFT UP and DOWN
                                                        +  grid[row_index-1][column_index-1],
                (_,_) => neighbour_amount               += grid[row_index+1][column_index+1]    //ADD ALL
                                                        + grid[row_index+1][column_index-1]
                                                        + grid[row_index-1][column_index+1]
                                                        + grid[row_index-1][column_index-1], 
            }
...
fn draw(grid:&Vec<Vec<i32>>){
    let height_scale = GRID_HEIGHT as f32/screen_height();
    let width_scale = GRID_WIDTH as f32/screen_width();
    let mut row_index:f32 = 0.0;
    let mut col_index:f32 = 0.0;
    for row in grid {
        for column in row{
            if *column == 1{
                draw_rectangle(col_index*width_scale, row_index*height_scale, 10.0*width_scale, 10.0*height_scale, BLACK);
            }
            col_index += 1.0;
        }
        col_index = 0.0;
